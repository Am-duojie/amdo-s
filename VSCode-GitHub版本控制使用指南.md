...
### 3. 在 GitHub 上查看文件历史版本 ⭐

GitHub 提供了强大的历史版本查看功能，你可以在网页上直接查看任何文件的历史版本。

#### 方法一：查看单个文件的历史（最常用）

1. **打开文件页面**：
   - 访问你的 GitHub 仓库：`https://github.com/Am-duojie/----`
   - 点击进入你想要查看的文件（例如：`backend/估价算法说明.md`）

2. **查看历史记录**：
   - 点击文件右上角的 **"History"** 按钮（时钟图标 ⏰）
   - 或点击 **"Blame"** 查看每一行是谁在什么时候修改的

3. **浏览不同版本**：
   - 你会看到所有修改过这个文件的提交记录
   - 每个提交显示：提交信息、提交者、提交时间
   - 点击任意一个提交，可以查看该提交时文件的内容

4. **查看具体版本**：
   - 在提交列表中，点击任意提交的提交信息
   - 会显示该提交的完整内容，包括哪些文件被修改了
   - 点击文件名，可以看到该文件在该提交时的完整内容

#### 方法二：查看所有提交历史

1. **访问提交页面**：
   - 在仓库主页，点击顶部的 **"commits"** 链接
   - 或直接访问：`https://github.com/Am-duojie/----/commits/main`

2. **浏览提交记录**：
   - 看到所有提交的列表
   - 每个提交显示：提交信息、修改的文件数量、提交时间和提交者
   - 点击提交信息，查看该提交修改了哪些文件和具体内容

3. **查看文件在某个提交时的状态**：
   - 点击提交后，可以看到所有被修改的文件
   - 点击文件名旁边的 "View" 查看完整文件内容
   - 或点击文件右侧的 `+` 和 `-` 查看具体修改
...
2. **查看历史**：
   - 点击文件进入文件详情页
   - 点击 **"History"** 查看所有版本
   - 或者点击文件右上角的三个点 `···` → 选择 "View file"

3. **切换到特定提交的文件版本**：
   - 在文件页面，点击文件路径上方的分支/提交哈希
   - 可以输入提交哈希或选择分支查看不同版本
   - 或者直接在 URL 中修改，例如：
     ```
...
✅ **对比两个版本**：
- 在文件历史页面，选择两个提交
- 点击 "Compare" 可以看到两个版本之间的差异
- 绿色表示新增，红色表示删除

✅ **搜索提交信息**：
- 在 commits 页面，可以使用搜索框
- 输入关键词搜索相关的提交记录
