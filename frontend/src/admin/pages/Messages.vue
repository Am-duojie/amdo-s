<template>
  <div>
    ...
    <el-table :data="items" style="width: 100%">
      ...
      <el-table-column prop="product" label="关联商品" width="200" />
      <el-table-column prop="content" label="消息内容" min-width="300" show-overflow-tooltip />
      <el-table-column prop="is_read" label="已读" width="100">
        <template #default="{ row }">
          <el-tag :type="row.is_read ? 'success' : 'info'">{{ row.is_read ? '是' : '否' }}</el-tag>
        </template>
      </el-table-column>
      <el-table-column prop="created_at" label="创建时间" width="180" />
      <el-table-column label="操作" width="120" fixed="right">
        <template #default="{ row }">
          <el-button v-if="hasPerm('message:delete')" size="small" type="danger" @click="remove(row)">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
...
const remove = async (row) => {
  try {
    ...
    await adminApi.delete(`/messages/${row.id}`)
