...
from app.secondhand_app.models import VerifiedProduct, Category
...
def main():
    ...
    if created:
        ...
        seller.save()
        print(f'✓ 创建测试卖家: {seller.username}')
    else:
        print(f'✓ 使用现有卖家: {seller.username}')
...
🔒 180天质保''',
    ...
    for idx, data in enumerate(products_data, 1):
        ...
        with connection.cursor() as cursor:
            cursor.execute(
                ...
                (seller_id, category_id, title, description, price, original_price, 
                 `condition`, status, location, brand, model, storage, 
                 battery_health, inspection_result, inspection_note, 
                 stock, tags, pricing_coefficient, source_tag, recycle_final_price, 
                 source_recycle_order_id, created_at, updated_at, view_count, sales_count, 
                 verified_at, inspection_date, inspection_staff, published_at, cover_image, 
                 detail_images, inspection_reports, contact_phone, contact_wechat, shop_id, 
                 verified_by_id, removed_reason)
                VALUES 
                (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, 
                 1.0, 'manual', NULL, NULL, NOW(), NOW(), %s, %s, %s, %s, %s, %s, '', '[]', '[]', 
                 '', '', NULL, NULL, '')
                """,
                [
                    ...
                    data['storage'], data['battery_health'],
        ...
        # 添加封面图（使用占位图）
        product.cover_image = f'https://via.placeholder.com/600x600/667eea/ffffff?text={product.brand}+{product.model.replace(" ", "+")}'
        ...
        print(f'{idx}. ✓ 创建商品: {product.title}')
        print(f'   价格: ¥{product.price} | 库存: {product.stock} | 成色: {product.get_condition_display()}')

    print('\n' + '=' * 60)
    print(f'完成！')
    print(f'成功创建: {created_count} 个商品')
    print(f'已存在跳过: {skipped_count} 个商品')
    print('=' * 60)
    print('\n可以访问前端页面查看效果：')
    print('http://localhost:5173/verified-products')
    print('=' * 60)
