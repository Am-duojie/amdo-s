...
# 回收：在线推理（建议价/风险）
@method_decorator(csrf_exempt, name='dispatch')
class RecycleMLPredictView(APIView):
    authentication_classes = []
    permission_classes = []
    ...
    def get(self, request):
        ...
        if not order_id:
            return Response({'success': False, 'detail': 'order_id 参数不能为空'}, status=400)

        try:
            order_id_int = int(order_id)
        except (TypeError, ValueError):
            return Response({'success': False, 'detail': 'order_id 参数格式错误'}, status=400)

        try:
            o = RecycleOrder.objects.select_related('template').get(id=order_id_int)
        ...
        if rep:
            report_data = {
                'check_items': rep.check_items,
                'remarks': rep.remarks,
                'evidence': rep.evidence,
                'overall_result': rep.overall_result,
                'recommend_price': rep.recommend_price,
                'score': rep.score,
            }

        try:
            from app.secondhand_app.ml.recycle_online_inference import predict_recycle_order
            prediction = predict_recycle_order(o, report=report_data)
        except Exception as e:
            return Response({'success': False, 'detail': f'在线推理失败: {str(e)}'}, status=500)

        return Response({'success': True, 'order_id': o.id, 'ml': prediction})
...
@method_decorator(csrf_exempt, name='dispatch')
class RecycleMLBatchView(APIView):
    """批量在线推理（分页列表）"""
    authentication_classes = []
    permission_classes = []
    ...
    def get(self, request):
        ...
        page_size = _parse_int(request.query_params.get('page_size', '20'), 20)
        page = max(1, page)
        page_size = max(1, min(100, page_size))

        statuses = _parse_csv_param(request.query_params.get('statuses', ''))
        date_from = _parse_date(request.query_params.get('date_from', ''))
        date_to = _parse_date(request.query_params.get('date_to', ''))

        qs = RecycleOrder.objects.select_related('template', 'user').all().order_by('-created_at')
        if statuses:
        ...
        # fetch latest reports for these orders
        order_ids = [o.id for o in items]
        reports_by_order = {}
        if order_ids:
            rep_qs = AdminInspectionReport.objects.filter(order_id__in=order_ids).order_by('order_id', '-id')
            for rep in rep_qs:
                if rep.order_id not in reports_by_order:
                    reports_by_order[rep.order_id] = rep

        from app.secondhand_app.ml.recycle_online_inference import predict_recycle_order

        results = []
        for o in items:
            rep = reports_by_order.get(o.id)
            pred = predict_recycle_order(o, report={'check_items': rep.check_items} if rep else None)
            estimated = float(o.estimated_price) if o.estimated_price else None
            gap_ratio = None
            if estimated and estimated > 0:
                try:
                    gap_ratio = abs(float(pred.get('suggested_final_price') or 0) - estimated) / estimated
                except Exception:
                    gap_ratio = None
            results.append({
                'id': o.id,
...
@method_decorator(csrf_exempt, name='dispatch')
class RecycleMLSummaryView(APIView):
    """批量在线推理（聚合统计/分布/TopN）"""
    authentication_classes = []
    permission_classes = []
    ...
    def get(self, request):
        ...
        date_from = _parse_date(request.query_params.get('date_from', ''))
        date_to = _parse_date(request.query_params.get('date_to', ''))
        max_items = _parse_int(request.query_params.get('max_items', '2000'), 2000)
        ...
        # status distribution from DB (full)
        from django.db.models import Count
        status_dist = list(qs.values('status').annotate(count=Count('id')).order_by('-count'))

        from app.secondhand_app.ml.recycle_online_inference import predict_recycle_order

        # Iterate for ML aggregates (sampled by max_items for performance)
        sampled_qs = qs[:max_items]
        order_ids = list(sampled_qs.values_list('id', flat=True))
        reports_by_order = {}
        if order_ids:
            rep_qs = AdminInspectionReport.objects.filter(order_id__in=order_ids).order_by('order_id', '-id')
            for rep in rep_qs:
        ...
        bins = [i / 10 for i in range(11)]  # 0.0..1.0
        risk_dispute_bins = [0] * 10
        risk_cancel_bins = [0] * 10
        gap_bins = [0] * 10  # 0~100% (cap at 100%)

        sum_suggested = 0.0
        sum_risk_dispute = 0.0
        sum_risk_cancel = 0.0
        suggested_count = 0
        ...
        sampled_orders = list(sampled_qs.select_related('template'))
        for o in sampled_orders:
            rep = reports_by_order.get(o.id)
            pred = predict_recycle_order(o, report={'check_items': rep.check_items} if rep else None)
            r1 = float(pred.get('risk_dispute') or 0.0)
            r2 = float(pred.get('risk_cancel') or 0.0)
            sp = float(pred.get('suggested_final_price') or 0.0)
            ...
            idx1 = min(9, max(0, int(r1 * 10)))
            idx2 = min(9, max(0, int(r2 * 10)))
            risk_dispute_bins[idx1] += 1
            risk_cancel_bins[idx2] += 1

            estimated = float(o.estimated_price) if o.estimated_price else 0.0
            if estimated > 0:
                gap_ratio = abs(sp - estimated) / estimated
                idxg = min(9, max(0, int(min(gap_ratio, 1.0) * 10)))
                gap_bins[idxg] += 1

            brand_key = o.brand or '-'
            brand_bucket = by_brand.get(brand_key)
            if not brand_bucket:
                brand_bucket = {'count': 0, 'sum_risk_dispute': 0.0, 'sum_gap_ratio': 0.0, 'gap_count': 0}
                by_brand[brand_key] = brand_bucket
            brand_bucket['count'] += 1
            brand_bucket['sum_risk_dispute'] += r1
            if estimated > 0:
                brand_bucket['sum_gap_ratio'] += abs(sp - estimated) / estimated
                brand_bucket['gap_count'] += 1
            ...
            high_risk.append({
                ...
                'risk_dispute': r1,
                'risk_cancel': r2,
                'suggested_final_price': sp,
            })

        high_risk.sort(key=lambda x: (x['risk_dispute'], x['risk_cancel']), reverse=True)
        high_risk = high_risk[:10]

        brand_rank = []
        for brand, b in by_brand.items():
            avg_risk = (b['sum_risk_dispute'] / b['count']) if b['count'] else 0.0
            avg_gap = (b['sum_gap_ratio'] / b['gap_count']) if b['gap_count'] else 0.0
            brand_rank.append({'brand': brand, 'count': b['count'], 'avg_risk_dispute': avg_risk, 'avg_gap_ratio': avg_gap})
        brand_rank.sort(key=lambda x: (x['avg_risk_dispute'], x['count']), reverse=True)
        brand_rank = brand_rank[:10]

        avg_suggested = (sum_suggested / suggested_count) if suggested_count else 0.0
        avg_risk_dispute = (sum_risk_dispute / suggested_count) if suggested_count else 0.0
        avg_risk_cancel = (sum_risk_cancel / suggested_count) if suggested_count else 0.0

        return Response({
            'success': True,
            'total': total,
            'sampled': len(sampled_orders),
            'filters': {
                'statuses': statuses,
            ...
            'avg': {
                'suggested_final_price': avg_suggested,
                'risk_dispute': avg_risk_dispute,
                'risk_cancel': avg_risk_cancel,
            },
            'distributions': {
                'risk_dispute_bins': {'bins': bins, 'counts': risk_dispute_bins},
                'risk_cancel_bins': {'bins': bins, 'counts': risk_cancel_bins},
                'gap_ratio_bins': {'bins': bins, 'counts': gap_bins},
            },
            'top': {
                'high_risk_orders': high_risk,
                'brands_by_risk': brand_rank,
            },
        })
...
class InspectionOrderDetailView(APIView):
    ...
    def get(self, request, order_id):
        ...
        # 在线推理（建议价/风险）——用于管理端“随时分析”
        ml_prediction = None
        try:
            from app.secondhand_app.ml.recycle_online_inference import predict_recycle_order
            # 将 report_data 以统一结构传入（只用到 check_items）
            ml_prediction = predict_recycle_order(o, report={'check_items': report_data.get('check_items')})
        except Exception:
            ml_prediction = None
